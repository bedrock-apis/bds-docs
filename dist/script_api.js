...
function GenerateItemComponentsRegistry() {
	...
	for (const itemType of mc.ItemTypes.getAll()) {
		...
		if (itemType.id === "minecraft:air") continue;
...
function GenerateEntityComponentRegistry() {
	...
	for (const entityType of mc.EntityTypes.getAll()) try {
		const entity = overworld.spawnEntity(entityType.id, {
			x: 0,
			y: 0,
			z: 0
		});
...
function GenerateTypesRegistry() {
	/** @type {Record<string, unknown>} */
	const typesRegistry = {};
	for (const [name, types] of Object.entries(mc)) if (name.endsWith("Types") && typeof types === "function" && "getAll" in types) typesRegistry[name] = types.getAll();
...
async function UploadMetadata(metadataKey, object) {
	...
	return net.http.request(new net.HttpRequest("http://localhost:9595/" + metadataKey).setBody(JSON.stringify(object)));
}
...
function ClassToJson(object) {
	...
	for (const property in object) {
		...
		if (value && typeof value === "object") if (Array.isArray(value)) newObject[property] = value.map(ClassToJson);
