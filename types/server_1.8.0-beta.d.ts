...
export class Block { private constructor(); readonly dimension: Dimension; readonly isAir: number; readonly isLiquid: number; readonly isSolid: number; isWaterlogged: number; readonly location: Vector3; readonly permutation: BlockPermutation; readonly 'type': BlockType; readonly typeId: string; readonly x: number; readonly y: number; readonly z: number; above(steps?: number): Block; below(steps?: number): Block; bottomCenter(): Vector3; canPlace(blockToPlace: BlockPermutation | BlockType | string, faceToPlaceOn?: Direction): number; center(): Vector3; east(steps?: number): Block; getComponent(componentId: string): BlockComponent; getItemStack(amount?: number, withData?: number): ItemStack; getRedstonePower(): number; getTags(): string[]; hasTag(tag: string): number; isValid(): number; north(steps?: number): Block; offset(offset: Vector3): Block; setPermutation(permutation: BlockPermutation): void; setType(blockType: BlockType | string): void; south(steps?: number): Block; trySetPermutation(permutation: BlockPermutation): number; west(steps?: number): Block}
...
export class EffectAddBeforeEvent { private constructor(); cancel: number; duration: number; readonly effectType: string; readonly entity: Entity}
