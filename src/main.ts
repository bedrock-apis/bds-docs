...
async function Main(): Promise<number> {
   ...
   if (exeSuccessful.exitCode) return Panic('BDS execution failed: ' + exeSuccessful.error);
   ...
   if (!editor.manifest.dependencies.find(e => e.module_name === SCRIPT_API_SERVER_NET_MODULE_NAME)) {
      ...
      if (failed) return Panic('Failed to overwrite new permissions for: ' + editor.manifest_file);
   }
   ...
   if (!permissions.data.allowed_modules?.includes(SCRIPT_API_SERVER_NET_MODULE_NAME)) {
      ...
      if (failed) return Panic('Failed to overwrite new permissions for: ' + permissions.permissionsFile);
   }
   ...
   // Spawn new process
   const { promise: runningProcessTask, child: runningProcess } = await InvokeProcessRaw(
   ...
   for await (const payload of server) {
      ...
      if (IsPacketTypeOf(data, PacketTypes.ScriptData)) {
         const { body } = data;
         await WriteFile(resolve(REPORTS_DIR_NAME, 'api.json'), JSON.stringify(body, null, 4));
      }

      if (IsPacketTypeOf(data, PacketTypes.BlocksData)) {
         const { body } = data;
         await WriteFile(resolve(REPORTS_DIR_NAME, 'blocks.json'), JSON.stringify(body, null, 4));
      }
   ...
   if (failed) return Panic('Script Failed to quit successfully, errorCode: ' + failed);
   ...
   failed = await createPost('1150152535475240991', 'New Update - ' + checkResults.version, 'Hello World: 🧪').then(
      () => 0,
      Panic,
   );
