...
import {
	...
	BDS_OUTDIR_PATH,
	DEBUG,
	FILE_CONTENT_BDS_TEST_CONFIG,
	FILE_CONTENT_CURRENT_EXIST,
	FILE_CONTENT_GITIGNORE,
	FILE_NAME_BDS_BINARY,
	FILE_NAME_BDS_TEST_CONFIG,
	FILE_NAME_GITHUB_README,
	FILE_NAME_GITHUB_REPO_EXISTS,
	FILE_NAME_GITIGNORE,
	IS_GITHUB_ACTION,
	LINK_BDS_CDN,
	LINK_BDS_VERSIONS,
	PLATFORM,
	REPORTS_DIR_NAME,
	SCRIPT_API_GEN_TIMEOUT,
	SCRIPT_API_SERVER_NET_MODULE_NAME,
...
Main()
	...
	.then((c) => {
		groupFinish();
		console.log(
			`Execution time: ${~~((Date.now() - performanceTime) / 1000)}s`
		);
		process.exit(c);
	});
...
async function Main(): Promise<number> {
	...
	FILE_CONTENT_CURRENT_EXIST["version"] = checkResults.usePreview
		? checkResults.version
		: GetEngineVersion(checkResults.version);
	...
	if (failed) return Panic(`Failed to checkout branch: ${checkResults.branch}`);
	groupEnd();
	...
	if (failed) return Panic("Failed to save workspace content");
	...
	if (failed) return Panic("Failed to write .gitignore file");

	// Fetch BDS Content
	group(`Download of Bedrock Dedicated Server -> ${BDS_OUTDIR_PATH}`);
	failed = await DownloadZipFile(
		`${LINK_BDS_CDN}/bin-${PLATFORM}${
			checkResults.usePreview ? "-preview" : ""
		}/bedrock-server-${checkResults.version}.zip`,
		BDS_OUTDIR_PATH
	);
	if (failed) return Panic("Failed to download BDS");
	...
	failed = await WriteFile(
		resolve(BDS_OUTDIR_PATH, FILE_NAME_BDS_TEST_CONFIG),
		FILE_CONTENT_BDS_TEST_CONFIG
	);
	if (failed) return Panic("Failed to write test_config.json");

	// Execute BDS Executable
	group(`BDS Static Generation -> ${BDS_OUTDIR_PATH}`);
	let exeSuccessful = await InvokeProcess(
		resolve(BDS_OUTDIR_PATH, FILE_NAME_BDS_BINARY),
		[],
		60_000
	); //1min limit
	...
	failed = await rm(resolve(BDS_OUTDIR_PATH, FILE_NAME_BDS_TEST_CONFIG)).then(
		() => 0,
		Panic
	);
	if (failed) return Panic("Failed to remove: " + FILE_NAME_BDS_TEST_CONFIG);
	...
	if (!code) return Panic("Failed to get script_api code for injection");

	// Get Extension to inject code in
	const editor = await GetEditorExtension(BDS_OUTDIR_PATH);
	if (!editor) return Panic("Failed to get editor extension");

	if (
		!editor.manifest.dependencies.find(
			(e) => e.module_name === SCRIPT_API_SERVER_NET_MODULE_NAME
		)
	) {
		console.log(
			"Injected missing dependency for " + SCRIPT_API_SERVER_NET_MODULE_NAME
		);
		editor.manifest.dependencies.push({
			version: SCRIPT_API_SERVER_NET_MODULE_VERSION,
			module_name: SCRIPT_API_SERVER_NET_MODULE_NAME,
		});

		failed = await WriteFile(
			editor.manifest_file,
			JSON.stringify(editor.manifest)
		);
		if (failed)
			return Panic(
				"Failed to overwrite new permissions for: " + editor.manifest_file
			);
	}

	failed = await WriteFile(editor.entry, code);
	if (failed) return Panic("Failed to inject code to: " + editor.entry);
	Success(
		"Successfully injected code for Editor extension to: " + editor.entry
	);
	...
	if (
		...
		if (failed)
			return Panic(
				"Failed to overwrite new permissions for: " +
					permissions.permissionsFile
			);
	}
	...
	// Spawn new process
	const { promise: runningProcessTask, child: runningProcess } =
		await InvokeProcessRaw(
			resolve(BDS_OUTDIR_PATH, FILE_NAME_BDS_BINARY),
			["Editor=true"],
			SCRIPT_API_GEN_TIMEOUT
		); //10s limit
	...
	for await (const payload of server) {
		...
		if (IsPacketTypeOf(data, PacketTypes.ErrorMessages)) {
			const { body } = data;
			await WriteFile(
				resolve(REPORTS_DIR_NAME, "errors.json"),
	...
	if (server.wasTerminatedByTimeout())
		return Panic(
			"Failed to generate dynamic information in time, webserver was terminated!"
		);
	...
	for (const flag of GENERATORS) {
		...
		if (failed) return Panic("Generator Failed " + flag.flagId);
	...
	if (failed) return Panic("Failed to write exist.json file");
	...
	failed = await createPost(
		"1150152535475240991",
		"New Update - " + checkResults.version,
		"Hello World: 🧪"
	).then(() => 0, Panic);
	if (failed) return Panic("Failed to post on discord");

	// Commit changes and force push
	group("Commit & Push -> " + checkResults.branch);
	failed = await GithubCommitAndPush(
		checkResults.branch,
		checkResults.version,
		checkResults.usePreview
	);
	if (failed) return Panic("Failed to commit and push");
	...
	failed = await GithubPostNewBranch(
		`${checkResults.branch}-${GetEngineVersion(checkResults.version)}`
	);
	if (failed) return Panic("Failed to post new branch");
