...
class ScriptModule {
   constructor(base) {
      ...
      dependencies.forEach((e, i) => {
         e.AKA = randomAliaes[i];
         this.dependencies[e.name] = e;
         this.dependenciesNames.push(e.name);
      });
   }
   /** @type {ModuleConstant[]} */
   constants;
...
class Type {
   constructor(type) {
      const { is_bind_type, is_errorable, name } = type;
      this.name = name;
   ...
   toString(m) {
      console.warn('never type!', this.name);
      return 'never';
   }
   fromValue(m, v) {
...
class UndefinedType extends Type {
   toString(m) {
      return this.isReturnType ? 'void' : 'undefined';
   }
}
...
class RecordType extends Type {
   ...
   toString(m) {
      return `Record<${this.keyType.toString(m)},${this.valueType.toString(m)}>`;
   }
}
class PromiseType extends ComplexType {
   ...
   toString(m) {
      return `Promise<${this.type.toString(m)}>`;
   }
}
class GeneratorType extends Type {
   ...
   toString(m) {
      return `Generator<${this.nextType.toString(m)},${this.returnType.toString(m)},${this.yieldType.toString(m)}>`;
   }
}
class ClosureType extends Type {
   ...
   toString(m) {
      return `(${this.argumentTypes.map((a, i) => `arg${i}${a instanceof OptionalType ? '?' : ''}: ${a.toString(m)}`)})=>${this.returnType.toString(m)}`;
   }
}
...
class VariantType extends Type {
   ...
   toString(m) {
      return '(' + this.types.map(e => e.toString(m)).join(' | ') + ')';
   }
}
...
class ModuleInterface extends ModuleMemberHasProperties {
   exportFrom(m) {
      return `export interface ${this.name} {${this.properties.length ? `${this.properties.map(e => `${e.isStatic ? 'static ' : ''}${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.type.toString(m)}`).join('; ')};` : ' '}}`;
   }
}
class ModuleError extends ModuleMemberHasProperties {
   exportFrom(m) {
      return `export class ${this.name} extends Error { private constructor()${this.properties.length ? `, ${this.properties.map(e => `${e.isStatic ? 'static ' : ''}${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.type.toString(m)}`).join(', ')}` : ' '}}`;
   }
}
...
class ModuleObject extends ModuleValue {
   ...
   exportFrom(m) {
      return `export ${this.isReadOnly ? 'const' : 'var'} ${this.name}: ${this.type.toString(m)};`;
   }
}
class ModuleConstant extends ModuleValue {
   ...
   toString(m) {
      return KnownTypeContructors[typeof this.value].prototype.fromValue(m, this.value);
   }
   exportFrom(m) {
      return `export ${this.isReadOnly ? 'const' : 'var'} ${this.name} = ${this.toString(m)};`;
   }
}
...
class ModuleFunction extends ModuleValue {
   ...
   exportFrom(m) {
      return `export function ${this.name}(${this.buildParams(m)}): ${this.returnType.toString(m)}`;
   }
   buildParams(m) {
...
class ModuleClass extends ModuleMember {
   ...
   exportFrom(m) {
      ...
      const members = [
         ...
         ...staticConstants.map(
            e =>
               `static ${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.toString(m)}`,
         ),
         ...staticProperties.map(
            e =>
               `static ${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.type.toString(m)}`,
         ),
         ...staticFunctions.map(
            e => `static ${safePropertyName(e.name)}(${e.buildParams(m)}): ${e.returnType.toString(m)}`,
         ),
         ...otherConstants.map(
            e =>
               `${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.toString(m)}`,
         ),
         ...otherProperties.map(
            e =>
               `${e.isReadOnly ? 'readonly ' : ''}${safePropertyName(e.name)}${e.type instanceof OptionalType ? '?' : ''}: ${e.type.toString(m)}`,
         ),
         ...otherFunctions.map(e => `${safePropertyName(e.name)}(${e.buildParams(m)}): ${e.returnType.toString(m)}`),
      ];
      return (
         (this.baseTypes.length ? '//@ts-ignore allow class inheritance for native classes\n' : '') +
