...
export async function GithubPostNewBranch(branch: BranchKind | `${BranchKind}-${VersionEngine}`): Promise<number> {
    ...
    if(!loginResult) 
        return Panic(`Failed to login as ${LOGIN_AS_NAME} ${LOGIN_AS_EMAIL}`);
    ...
    if(result.exitCode != 0) 
        return Panic(`Fail to execute '${result.command + " " +  params.join(" ")}'  command`);
...
export async  function GithubCheckoutBranch(branch: BranchKind, force: boolean): Promise<number> {
    ...
    if(result.exitCode != 0) 
        return Panic(`Fail to execute '${result.fullCommand}' command`);
...
export async function GithubCommitAndPush(branch: BranchKind, version: VersionFull, isPreview: boolean): Promise<number> {
    if(DEBUG) {
        console.log("[DEBUG] Login Skipped . . .");
        return 0;
    }
