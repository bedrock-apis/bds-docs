...
export const GITHUB_STABLE_VERSIONED_BRANCH_NAME: <T extends VersionEngine>(
   engine: T,
) => `${typeof GITHUB_STABLE_BRANCH_NAME}-${T}` = <T extends VersionEngine>(engine: T) =>
   `${GITHUB_STABLE_BRANCH_NAME}-${engine}`;
