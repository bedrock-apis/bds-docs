...
/**
 * @param {Partial<Omit<MD.MetadataType, 'name' | 'is_bind_type' | 'is_errorable'>> &
 *    Pick<MD.MetadataType, 'name' | 'is_bind_type' | 'is_errorable'>} object
...
function ClassConstant(data, context) {
   ...
   text +=
      'value' in data
         ? `${propertName(data.name)} = ${ToLiteral(data.value)}`
         : `${propertName(data.name)}: ${TypePrinter(data.type, context, true, false)}`;
   return text;
}
/**
