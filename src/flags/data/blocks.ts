...
function MapBlock(
   ...
   const lengths = block_item.properties.map(e => states_map[e]?.length).filter(e => typeof e === 'number');
...
function* PermutationGenerator(lengths: number[], index: number): Generator<number[]> {
   ...
   if (typeof length !== 'number') length = 0; // impossible
