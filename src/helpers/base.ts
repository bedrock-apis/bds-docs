import {
   ...
   GITHUB_PREVIEW_BRANCH_NAME,
   GITHUB_STABLE_BRANCH_NAME,
   LINK_GITHUB_REPO,
} from '../consts';
...
export async function GetRepositoryVersionIncompatibility(
   versions: BDSVersions,
): Promise<VersionIncompatibility | null> {
   ...
   if (!data || data.version !== GetEngineVersion(versions.windows.stable))
      return {
         branch: GITHUB_STABLE_BRANCH_NAME,
         version: versions.windows.stable,
         usePreview: false,
   ...
   if (!data || data.version !== versions.windows.preview)
      return {
