...
export interface TestRunResultSimple {
	type: "simple";
	result: TestRunResult;
}

export interface TestRunResultChain {
	type: "chain";
	result: TestRunResult[];
}
...
export function IsPacketTypeOf<T extends keyof PacketBodyType>(
	packet: unknown,
	type: T
): packet is IPacket<T> {
	return (
		typeof packet === "object" && (packet as { type: unknown })?.type === type
	);
}
